...
DEFAULT_OUT_DIR = Path("data") / "processed" / "policy_learning"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a tabular policy-learning dataset from 9-point first-pass "
            "features and dense-reference follow-up action outcomes."
        )
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=10.0,
        help="Penalty weight for severe misses in the action score.",
    )
    parser.add_argument(
        "--absolute-error-weight",
        type=float,
        default=1.0,
        help="Penalty weight for absolute defect-ratio error in the action score.",
    )
    parser.add_argument(
        "--underestimation-weight",
        type=float,
        default=0.25,
        help="Penalty weight for underestimating the dense-reference defect ratio.",
    )
    parser.add_argument(
        "--added-cost-weight",
        type=float,
        default=0.003,
        help="Penalty weight per added sampled valid die beyond the 9-point first pass.",
    )
...
def action_masks(wafer_map: np.ndarray, seed: int) -> dict[str, np.ndarray]:
    first = make_9point_mask(wafer_map)
    local = local_expand_mask(wafer_map, first)
    random16 = make_random_sampling_mask(wafer_map, n_points=16, seed=seed)
...
def evaluate_actions(
    ...
    for action, mask in action_masks(wafer_map, seed).items():
...
def main() -> None:
    ...
    for pos, row in enumerate(df.itertuples(index=True), start=1):
        ...
        seed = row_index + 20260623

        feature_records.append(
            first_pass_features(row_index, failure_type, wafer_map)
        )
        outcome_records.extend(
            evaluate_actions(row_index, failure_type, wafer_map, seed, args)
        )

        if pos % 10000 == 0 or pos == total:
            print(f"policy dataset rows processed: {pos:,}/{total:,}")
    ...
    print("score weights:")
    print(f"  severe_miss_weight={args.severe_miss_weight}")
    print(f"  absolute_error_weight={args.absolute_error_weight}")
    print(f"  underestimation_weight={args.underestimation_weight}")
    print(f"  added_cost_weight={args.added_cost_weight}")
